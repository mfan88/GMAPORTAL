...
export default async function LinkExpiredPage({
  ...
  const expiryHours = Math.max(1, Math.round(linkExpirySeconds(config) / 3600));
  ...
  if (isPending && availableAt) {
    ...
    mainContent = (
      <Alert className="mx-auto max-w-lg">
        ...
        <AlertDescription>
          If you believe this is an error, please request for a new upload link.
          Each link works once and is valid for {expiryHours} hours.
        </AlertDescription>
      </Alert>
    );
  }
